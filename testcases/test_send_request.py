...
class TestSendRequest:
    ...
    login_data = YamlUtil().read_tesecase_yaml('login.yml')
    print("这是数据", login_data)
    pw = login_data[0]["request"]["data"]["password"].split()
    @pytest.mark.parametrize("pw_encryption", login_data[0]["request"]["data"]["password"].split(), indirect=True)
    def test_login(self, pw_encryption):

        print("这是加密后的密码：", pw_encryption)
#         pw = pw_encryption
